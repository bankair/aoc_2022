...
def perform(input)
  ...
  input.split(/\n/).map(&:chomp).each do |line|
    ...
    when /^[1-9][0-9]* /
      files << file_infos = FileInfos.new(line)
      current.entries[file_infos.name] = file_infos
    else
      raise "Parsing error: #{line}" unless line == '$ ls'
    end
  end
  space_needed = files.map(&:size).sum - 40_000_000
  directories.map(&:size).sort.find { |size| size >= space_needed }
end

should_return(24_933_642).for do
  perform(<<~INPUT)
    $ cd /
    $ ls
    dir a
